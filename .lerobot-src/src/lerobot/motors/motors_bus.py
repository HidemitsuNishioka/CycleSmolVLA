...
class SerialMotorsBus(MotorsBusBase):
    ...
    def _connect(self, handshake: bool = True) -> None:
        try:
            ...
            # openPort() succeeds even when the USB-UART adapter is present but
            # the motor bus is unpowered/disconnected.  Close the port here so
            # callers do not enter disconnect() with a half-connected bus and
            # try to write Torque_Enable to motors that never answered.
            ...
            # Motor handshake failures (for example, missing motor IDs) are
            # intentionally re-raised unchanged, but the serial handle must
            # still be released before the caller's cleanup path runs.
            if self.port_handler.is_open:
                self.port_handler.closePort()
            raise
