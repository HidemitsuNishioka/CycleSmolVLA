...
def prepare_observation_for_inference(
    ...
    Args:
        observation: A dictionary mapping observation names (str) to NumPy
            array data. Images can be (H, W, C) or temporal windows (T, H, W, C).
    ...
    for name in observation:
        ...
        if "image" in name and not name.endswith(("_is_pad", "_padding_mask")):
            if tensor.dtype == torch.uint8:
                tensor = tensor.type(torch.float32) / 255
            tensor = tensor.movedim(-1, -3).contiguous()
