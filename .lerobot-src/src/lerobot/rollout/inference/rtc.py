...
class RTCInferenceEngine(InferenceEngine):
    ...
    def __init__(
        ...
        self._obs_lock = Lock()
        self._observation_history = ObservationHistory.from_config(getattr(policy, "config", None))
    ...
    def stop(self) -> None:
        ...
        if self._rtc_thread is not None and self._rtc_thread.is_alive():
            ...
            if self._rtc_thread.is_alive():
                # A restart must never reset a model still being used by this worker.
                # Keep its handle so cleanup can retry; do not permit a second worker.
                raise TimeoutError(f"RTC thread did not stop within {_RTC_JOIN_TIMEOUT_S:.1f}s")
            else:
        ...
        try:
            ...
            while not self._shutdown_event.is_set():
                ...
                with self._obs_lock:
                    ...
                    history = (
                        self._observation_history.snapshot()
                        if self._observation_history is not None
                        else None
                    )
                ...
                if queue.qsize() <= self._rtc_queue_threshold:
                    try:
                        ...
                        obs_batch = (
                            ObservationHistory.stack(history)
                            if history is not None
                            else build_dataset_frame(self._hw_features, obs, prefix="observation")
                        )
                        obs_batch = prepare_observation_for_inference(
