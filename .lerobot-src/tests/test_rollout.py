...
def test_base_keyboard_restart_drops_interpolation_and_refreshes_observation():
    from lerobot.rollout import BaseStrategyConfig
    from lerobot.rollout.strategies import BaseStrategy
    from lerobot.utils.action_interpolator import ActionInterpolator

    strategy = BaseStrategy(BaseStrategyConfig())
    ctx, _ = _make_loop_ctx(
        fps=200.0, multiplier=2, num_ticks=6,
        on_tick=lambda tick: strategy._on_key("R") if tick == 2 else None,
    )
    engine = ctx.policy.inference
    strategy._engine = engine
    strategy._interpolator = ActionInterpolator(multiplier=2)
    strategy.run(ctx)
    lifecycle = [call[0] for call in engine.mock_calls if call[0] in {"pause", "stop", "reset", "start", "resume"}]
    assert lifecycle == ["resume", "pause", "stop", "reset", "start", "resume"]
    # Restart arrives midway through an interpolation cycle: tick 3 must use a
    # fresh inference and fresh observation, not the old endpoint/interpolant.
    observations = [call.args[0]["m.pos"] for call in engine.notify_observation.call_args_list]
    assert observations == [1.0, 2.0, 3.0, 4.0, 6.0]
    sent = [call.args[0]["m.pos"] for call in ctx.hardware.robot_wrapper.send_action.call_args_list]
    assert sent[2] == 3.0
    ctx.hardware.robot_wrapper.connect.assert_not_called()
    ctx.hardware.robot_wrapper.disconnect.assert_not_called()


def test_base_restart_resets_duration_clock(monkeypatch):
    from lerobot.rollout import BaseStrategyConfig
    from lerobot.rollout.strategies import BaseStrategy
    from lerobot.rollout.strategies import base
    from lerobot.utils.action_interpolator import ActionInterpolator
    ...
    ctx, _ = _make_loop_ctx(fps=30, multiplier=1, num_ticks=10, on_tick=tick)
    ctx.runtime.cfg.duration = 1.0
    strategy._engine = ctx.policy.inference
    strategy._interpolator = ActionInterpolator(multiplier=1)
    monkeypatch.setattr(base.time, "perf_counter", lambda: clock["now"])
    monkeypatch.setattr(base, "CycleTimer", MagicMock())
    strategy.run(ctx)
    # Without renewing the deadline it would end after tick 2 at 1.2 seconds.
    assert ctx.hardware.robot_wrapper.get_observation.call_count == 3
    base.CycleTimer.return_value.restart.assert_called_once()


@pytest.mark.parametrize("interactive", [False, True])
def test_base_terminal_listener_cleanup_and_interactive_ownership(monkeypatch, interactive):
    from lerobot.rollout import BaseStrategyConfig
    from lerobot.rollout.strategies import BaseStrategy
    from lerobot.rollout.strategies import base
    from lerobot.utils.action_interpolator import ActionInterpolator

    ctx, _ = _make_loop_ctx(fps=30, multiplier=1, num_ticks=1)
    ctx.runtime.cfg.interactive = interactive
    ctx.hardware.robot_wrapper.get_observation.side_effect = RuntimeError("camera failed")
    strategy = BaseStrategy(BaseStrategyConfig(keyboard_restart=True))
    strategy._engine = ctx.policy.inference
    strategy._interpolator = ActionInterpolator(multiplier=1)
    listener = MagicMock()
    monkeypatch.setattr(base, "TerminalKeyListener", listener)
    monkeypatch.setattr(base.sys.stdin, "isatty", lambda: True)
...
def test_base_restart_does_not_reset_or_resume_after_worker_stop_failure():
    from lerobot.rollout import BaseStrategyConfig
    from lerobot.rollout.strategies import BaseStrategy
    ...
    engine = ctx.policy.inference
    strategy._engine = engine
    engine.stop.side_effect = TimeoutError("worker still running")
    with pytest.raises(TimeoutError):
        strategy._restart_inference(ctx, MagicMock())
